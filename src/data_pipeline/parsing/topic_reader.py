...
class Topic:
    ...
    def __init__(self, topic_id, title, question, tags, post_id=None):
        self.topic_id = topic_id
        self.title = title
        self.question = question
        self.lst_tags = tags
        self.post_id = int(post_id) if post_id and str(post_id).isdigit() else None
...
class TopicReader:
    ...
    def __read_topics(self, topic_file_path):
        ...
        for child in root:
            ...
            post_id  = child.attrib.get('post_id')
            title    = child[0].text
            question = child[1].text
            lst_tag  = child[2].text.split(",")
            map_topics[topic_id] = Topic(topic_id, title, question, lst_tag, post_id)
        return map_topics
